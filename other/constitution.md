...
- **Data sources:** Pascal case. A SharePoint list's name becomes its identifier in Power Fx and cannot be renamed from within the application.
- **Screens:** governed by Principle V.

**Open item:** `docs/LDP_Logical_Data_Model.md` currently names lists in upper snake case, for example `APPLICATION_PROGRAM_CHOICES`. This conflicts with the Pascal case data source convention, and it cannot be corrected once lists are created and referenced in formulas. Reconcile before list creation.
