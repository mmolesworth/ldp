...
- The program's committee rating sheet exists and is published: its selected criteria, each with the four anchors (0, 1, 3, 5).
...
No partial or off-anchor score is recorded as final. Scores entered in one session are retained for the next; an abandoned session leaves prior recorded scores intact.
...
3. The system presents the program's rating sheet: each selected criterion with its four anchors (0, 1, 3, 5) and their descriptors.
4. The committee selects one anchor value for each criterion.
...
- **4a.** A value that is not one of the criterion's anchor values (0, 1, 3, 5) is submitted: the system rejects it and prompts for an anchor value (not recorded until valid).
...
1. DTD opens a program; the system presents that program's ranked applicants with, for each: the committee ranking, the applicant's percent of possible, the applicant's ranked program-option preferences, any supervisor recommendations, and whether they are already placed elsewhere.
...
### Criteria selection (per sheet version)

- The sheet's criteria are selected from the shared criterion catalog (UC-4.4), not authored here.
- Only a Published criterion within its availability window can be selected.
- Each criterion carries its own name, description, and four anchors from the catalog; the sheet does not restate them.
- Criteria sum to a total; the total is emergent (no imposed maximum), and equals five times the number of selected criteria.

### Main success scenario (build and publish)

1. DTD opens the rating sheet for a program and selects criteria from the catalog. Each selection references a Published, in-window criterion.
...
- **2a.** DTD attempts to save a sheet with no selected criteria: the system blocks the save. A sheet must contain at least one selected criterion to be saved as a version.
- **2b.** A criterion on the sheet has an end date that has passed: the system flags it and blocks publishing until DTD removes or replaces it.
...
## UC-4.4: Manage Rating Criterion Catalog
...
- **DTD Personnel:** wants one shared set of scoring criteria to draw from when building any program's rating sheet, with each criterion's wording stable once released so sheets across programs and cycles stay comparable.
- **Committee (indirect):** wants to score against criteria whose meaning does not shift after evaluation has begun.
...
The catalog contains the intended Published criteria, each with a name, a description, and four anchors (0, 1, 3, 5), optionally with an example per anchor. Published criteria are frozen and available for selection onto rating sheets (UC-4.3).

### Minimal guarantee

No published criterion changes. A criterion carries at least the four anchors before it can be published.

### Trigger

DTD opens criterion-catalog management.

### Criterion lifecycle

- **Draft** — DTD is building or revising the criterion. Editable without limit.
- **Published** — DTD has released the criterion. Frozen: its text does not change. A change is made by creating a new criterion. Available for selection onto rating sheets, subject to its availability window.

### Criterion structure

- A name and a description of what the rater evaluates.
- Four anchors at the fixed scores 0, 1, 3, and 5, each with descriptor text and an optional example. The four scores are the same for every criterion; there is no criterion-specific scale.
- A start date and an end date. A null end date means the criterion is still available.

### Main success scenario (define and publish)

1. DTD opens the catalog and defines a criterion: a name, a description, and the four anchor descriptors, optionally attaching an example to any anchor.
2. DTD saves. The system records the criterion as Draft.
3. DTD revises as needed while the criterion is Draft.
4. DTD publishes the criterion. The system freezes its text, records its start date, and makes it selectable onto rating sheets.
5. When a criterion should no longer be offered, DTD sets an end date; the system stops offering it for new selection from that date.

### Extensions

- **1a.** DTD attempts to publish a criterion missing one or more of the four anchors: the system blocks publishing until all four are present.
- **4a.** DTD needs to correct a Published criterion: the system does not allow editing; DTD creates a new criterion and, if appropriate, sets an end date on the old one.
- **5a.** A criterion's end date has passed but it still sits on a draft rating sheet: handled at rating-sheet publish (UC-4.3), which blocks publishing a sheet that carries an inactive criterion.
